  ...
  if(isset($_POST['registerUser'])) {
    ...
    if (strlen($fullname)<4 || strlen($email)<6 || strlen($address)<6 || strlen($username)<4 || strlen($pass)<4){
        ...
        if ( $statement->rowCount() == 0){
            ...
            if ($value){
                echo '<script>alert("Success Resgistration. Please select Connect in order to login")</script>';
            }
            else{
...
?>
  <div class="container">
    ...
    <form method="post" action="">
        ...
        <div class="form-group">
            <input type="submit" value="Register" class="btn btn-primary btn-lg btn-block" name="registerUser"></div>
    </form>
</div>
</body>
</html>